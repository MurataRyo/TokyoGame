...
public partial class LightShafts : MonoBehaviour
{
	...
	void UpdateShadowmap()
	{
		...
		else
		{
			...
			m_ShadowmapCamera.nearClipPlane = m_SpotNear * GetComponent<Light>().range;
			m_ShadowmapCamera.farClipPlane = m_SpotFar * GetComponent<Light>().range;
	...
	void RenderInterpolationTexture(Vector4 lightPos)
	{
		...
		if (!m_DX11Support && (Application.platform == RuntimePlatform.WindowsEditor))
		{
